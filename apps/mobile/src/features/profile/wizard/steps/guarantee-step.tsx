...
export function GuaranteeStep({ mode }: StepScreenProps) {
  ...
  const form = useForm({
    defaultValues: (profile
      ? (defaultsForStep('guarantee', profile) as { guaranteeCapacityCents: number })
      : { guaranteeCapacityCents: 0 }),
    validators: { onSubmit: guaranteeStepSchema },
    onSubmit: ({ value }) => submit(value),
  });

  if (!profile) return null;
  const { index, total } = applicableStepIndex('guarantee', profile);
  const copy = STEP_COPY.guarantee;
  ...
  return (
    ...
    >
      <form.Field name="guaranteeCapacityCents">
        {(field) => (
          <CurrencyField
            valueCents={field.state.value}
            onChangeCents={field.handleChange}
            label={t('profile.wizard.step.guarantee.field')}
